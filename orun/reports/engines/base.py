...
class ReportEngine(object):
    ...
    def query_to_xml(self, query, fields=None):
        template = """<?xml version="1.0" standalone="yes"?><DataSet>%s</DataSet>"""
        return template % ''.join(['<record>%s</record>' % ''.join(['<%s>%s</%s>' % (f, self._format_field(r, f), f) for f in fields]) for r in query])
