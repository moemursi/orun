...
from orun.db.migrations.utils import (
...
class MigrationAutodetector(object):
    ...
    def generate_created_models(self):
        ...
        for app_label, model_name in all_added_models:
            ...
            if primary_key_rel and not isinstance(primary_key_rel, str):
                dependencies.append((
    ...
    def generate_altered_db_table(self):
        ...
        for app_label, model_name in sorted(models_to_check):
            ...
            old_model_state = self.from_state.models[app_label, old_model_name]
            new_model_state = self.to_state.models[app_label, model_name]
            old_db_model_name = old_model_state.options.get('db_table')
            new_db_model_name = new_model_state.options.get('db_table')
            if old_db_model_name != new_db_model_name:
                self.add_operation(
                    app_label,
                    operations.AlterModelTable(
                        name=model_name,
                        table=new_db_model_name,
                    )
                )
    ...
    def get_module_migrations(self, addon):
        mig_mod = '%s.migrations' % addon.__class__.__module__
        versions = OrderedDict()
        try:
            mod = import_module(mig_mod)
            version_list = OrderedDict()
            initial = None
            for importer, mod_name, ispkg in pkgutil.iter_modules(mod.__path__):
                if mod_name == 'initial':
                    initial = 'initial'
                elif not mod_name.startswith('_') and ispkg:
                    mod_ref = (int(m) for m in mod_name.split('_'))
                    version_list[mod_ref] = mod_name
            keys = sorted(version_list.keys())
            if initial:
                keys = [initial] + list(keys)
            for k in keys:
                version_list[k] = mod_name
                migs = []
                versions[mod_ref] = migs
                mod_version = import_module('%s.migrations.%s' % (addon.__class__.__module__, mod_name))
                for _, mig_name, ispkg in pkgutil.iter_modules(mod_version.__path__):
                    if not ispkg:
                        migs.append(mig_name)
        except ImportError:
            return []
        return versions
