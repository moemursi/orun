...
MAX_NAME_LENGTH = 30  # Oracle max name length
...
class Options(object):
    ...
    def __init__(self, parents, original_model=None, app_label=None):
        ...
        self.db_tablespace = None  # settings.DEFAULT_TABLESPACE
        self.required_db_features = []
        self.required_db_vendor = None
        self.meta = None
        self.pk = None
        if 'abstract' not in self.__class__.__dict__:
            self.__class__.abstract = False
        if app_label:
        ...
        if self.original_model:
            for b in self.bases:
                b = b._meta
                if isinstance(b, Options) and b.has_auto_field:
                    self.__class__.auto_field = b.auto_field
                    self.__class__.has_auto_field = True
        ...
        if original_model:
            self.app_label = original_model._meta.app_label
        elif not hasattr(self, 'app_label'):
            self.app_label = None
    ...
    def contribute_to_class(self, cls, name):
        ...
        if self.app_label is None:
            ...
            self.app_label = mod_name
        else:
            mod_name = self.app_label
        if opts.name is None:
            if self.parents:
                opts.name = self.parents[0]._meta.name
            else:
                try:
                    s = apps.get_addon(mod_name).schema
                except KeyError:
                    s = mod_name.replace('_', '.')
                opts.name = '%s.%s' % (s, camel_case_to_spaces(cls.__name__).replace(' ', '.'))

        self.unique_together = normalize_together(self.unique_together)
        self.index_together = normalize_together(self.index_together)

        # First, construct the default values for these options.
        opts.object_name = cls.__name__
        if not opts.name:
            opts.name = self.object_name.lower()
        if not opts.verbose_name:
        ...
        if not opts.verbose_name_plural:
            opts.verbose_name_plural = string_concat(opts.verbose_name, 's')
        ...
        if not self.original_model and self.apps:
            try:
                opts.addon = apps.get_addon(self.app_label)
            except KeyError:
                pass
        ...
        if not self.db_table:
            ...
            if self.db_schema:
                ...
                opts.db_table = table_name
        elif self.original_model is None and '.' in self.db_table and '"."' not in self.db_table and self.db_table.startswith('"'):
            opts.db_table = '"%s"' % opts.db_table.replace('.', '"."')
    ...
    def _prepare(self, model):
        ...
        if self.pk is None:
            if self.parents:
                ...
                # field.
                field = next(self.parents.values())
                ...
                if not field.remote_field.parent_link:
                    warnings.warn(
                        ...
                        'deprecated.' % field, RemovedInOrun20Warning
                    )
    ...
    def managers(self):
        ...
        for depth, base in enumerate(bases):
            for manager in base._meta.local_managers:
                ...
                # remove afterwards. (RemovedInOrun20Warning)
    ...
    def _get_fields2(self, *args, **kwargs):
        return chain(self.local_fields, self.local_many_to_many)
        fields = []
        for parent in reversed(self.bases):
            if parent._meta.original_model:
                for f in parent._meta._get_fields():
                    fields.append(f)
            else:
