...
from orun.utils.functional import curry
...
class ModelBase(type):
    ...
    def __new__(cls, name, bases, attrs):
        app = attrs.pop('__app__', None)
        original_model = attrs.pop('__model__', None)
        parents = [b for b in bases if isinstance(b, ModelBase)]
        meta_class = attrs.get('Meta', None)
        abstract = getattr(meta_class, 'abstract', False)
        if app is None:
            bases = [b for b in bases if not isinstance(b, ModelBase) or b is Model]
            meta_parents = [b._meta.__class__ for b in parents if hasattr(b, '_meta')]
        else:
            if 'Meta' in parents[0].__dict__:
                meta_class = parents[0].Meta
            meta_parents = [b._meta.__class__ for b in parents if b._meta.original_model is not None and hasattr(b, '_meta')]
        if not meta_parents:
            meta_parents = [Options]
        if meta_class is not None:
            meta_parents.insert(0, meta_class)

        if not bases and parents:
            bases = [Model]

        if not parents:
            return super(ModelBase, cls).__new__(cls, name, tuple(bases), attrs)

        module = attrs.pop('__module__')
        new_class = super(ModelBase, cls).__new__(cls, name, tuple(bases), {'__module__': module})
        ...
        if app is None and not new_class._meta.extension and parents:
            ...
            for base in parents:
                if not hasattr(base, '_meta') or base == new_class:
                    continue
                if not base._meta.abstract:
                    ...
                    for field in parent_fields:
                        if field.name not in field_names and not field.primary_key:
            ...
            for base in reversed(parents):
                if hasattr(base, '_meta') and base._meta.parents:
                    new_class._meta.parents.update(base._meta.parents)

        if abstract:
            new_class.Meta.abstract = False
            return new_class
    ...
    def _build_model(cls, app):
        bases = [app[b._meta.name] if not b._meta.abstract else b for b in cls._meta.bases if isinstance(b, ModelBase) and b is not Model]
        attrs = {
            '__app__': app,
            '__model__': cls,
            '__module__': None,
        ...
        if bases:
            bases.insert(0, cls)
            new_class = type(cls.__name__, tuple(bases), attrs)
        else:
            new_class = type(cls.__name__, (cls,), attrs)
        return new_class
    ...
    def _prepare(cls):
        ...
        get_absolute_url_override = None#settings.ABSOLUTE_URL_OVERRIDES.get(opts.label_lower)
    ...
    def _requires_legacy_default_manager(cls):  # RemovedInOrun20Warning
        opts = cls._meta
...
class Model(metaclass=ModelBase):
    ...
    def to_dict(self, fields=None):
        if fields:
            return {f: self.serializable_value(f) for f in fields}
        else:
    ...
    @classmethod
    def toggle_active(cls, rows, auto_save=True):
        for row in rows:
    ...
    def __copy__(self):
        ...
        obj = {f.attname: getattr(self, f.attname) for f in model._meta.fields if f.copy}
