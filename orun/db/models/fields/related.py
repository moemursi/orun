...
from orun.db.models.deletion import CASCADE, SET_DEFAULT, SET_NULL
...
from orun.utils.functional import cached_property, curry
...
class OneToOneField(ForeignKey):
    ...
    def __init__(self, to, on_delete=None, to_field=None, **kwargs):
        ...
        if on_delete is None:
            warnings.warn(
                "on_delete will be a required arg for %s in Orun 2.0. Set "
                "it to models.CASCADE on models and in existing migrations "
                "if you want to maintain the current default behavior. "
                "See https://docs.orunproject.com/en/%s/ref/models/fields/"
                "#orun.db.models.ForeignKey.on_delete" % (
            ...
            warnings.warn(
                "The signature for {0} will change in Orun 2.0. "
...
def create_many_to_many_intermediary_model(field, klass):
    ...
    obj_name = '%s_%s' % (klass._meta.name, field.name)
    lazy_related_operation(set_managed, klass, to_model, name)

    to = make_model_tuple(to_model)
