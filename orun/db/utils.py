...
def load_backend(backend_name):
    ...
    """
    # This backend was renamed in Orun 1.9.
    if backend_name == 'orun.db.backends.postgresql_psycopg2':
        backend_name = 'orun.db.backends.postgresql'
    ...
    try:
        ...
        try:
            ...
            builtin_backends = []
        if backend_name not in ['orun.db.backends.%s' % b for b in
                                builtin_backends]:
            backend_reprs = map(repr, sorted(builtin_backends))
            error_msg = ("%r isn't an available database backend.\n"
                         "Try using 'orun.db.backends.XXX', where XXX "
                         "is one of:\n    %s\nError was: %s" %
