...
(function() {
    ...
    class WindowAction extends Action {
        ...
        async formButtonClick(self) {
            ...
            const res = await scope.model.post(meth, {
                kwargs: {
                    id: scope.record.id
                }
            });
            ...
            if (this.state === DataSourceState.inserting && Katrid.Settings.UI.goToDefaultViewAfterCancelInsert) {
                ...
                if (this.state === DataSourceState.editing) {
                    if (this.scope.record) {
                        ...
                        if (r && $.isFunction(r.promise)) r.then(() => {
                            this.state = DataSourceState.browsing;
                            for (let child of this.children) if (child.scope.masterChanged) {
                                child.scope.masterChanged(this.recordId);
                                child.scope.$apply();
                            }
                        }); else this.state = DataSourceState.browsing;
                    }
                } else {
                    this.record = {};
                    this.state = DataSourceState.browsing;
...
(function() {
    ...
    class Chart extends Katrid.UI.Widgets.Component {
        ...
        link(scope, el, attrs) {
            let r;
            if (_.isUndefined(attrs.url)) r = Katrid.Services.Query.read(attrs.queryId); else r = $.ajax({
                url: attrs.url,
                type: "get"
            });
            r.done(res => {
