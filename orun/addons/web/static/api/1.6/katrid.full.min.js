...
(function() {
    ...
    class Action {
        ...
        backTo(index, viewType) {
            if (this._unregisterHook && Katrid.Actions.actionManager.actions.length > 1) this._unregisterHook();
            let action = Katrid.Actions.actionManager.action = Katrid.Actions.actionManager.actions[index];
            if (index === 0 && viewType === 0) return action.restore(action.searchViewType);
    ...
    class WindowAction extends Action {
        ...
        doBindingAction(evt) {
            this.selection;
            Katrid.Services.Actions.load($(evt.currentTarget).data("id")).done(action => {
        ...
        autoReport() {
            return this.model.autoReport().done(function(res) {
...
(function() {
    ...
    ngApp.config(function($routeProvider) {
        return;
        $routeProvider.when("/client/action/:actionName/", {}).when("/action/:actionId/", {
            controller: "ActionController",
            reloadOnSearch: false,
            resolve: {
                action: [ "$route", $route => {
        ...
        link(scope, element, attrs, controller) {
            element.keypress(function(evt) {
                if (evt.key.toLowerCase() === "h") {
                    let value;
                    if (attrs["type"] === "date") value = new Date().toISOString().split("T")[0]; else value = new Date().toISOString().split(".")[0];
                    $(element).val(value);
    ...
    class View extends ActionView {
        getBreadcrumb() {
            ...
            for (let h of Katrid.Actions.actionManager.actions) {
                ...
                i++;
                if (Katrid.Actions.actionManager.actions.length > i && h.viewType === "form") html += `<li class="breadcrumb-item"><a href="javascript:void(0)" ng-click="action.backTo(${i - 1})">${h.scope.record.display_name}</a></li>`;
            }
...
(function() {
    class SearchMenu {
        ...
        expand(item) {
            const {scope: scope} = this.searchView;
            return scope.model.getFieldChoices(item.ref.name, scope.search.text).then(res => {
                if (res.ok) {
                    return Array.from(res.result).map(obj => this.searchView.loadItem(item.item, obj, item));
                }
            });
        }
        collapse(item) {
        ...
        reset() {
            for (let i of this.searchView.items) if (i.children && i.children.length) {
                this.collapse(i);
                result.push(i.reset());
            }
        }
