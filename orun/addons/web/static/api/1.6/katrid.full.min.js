    ...
    var methods = {
        ...
        init: function(settings) {
            ...
            return this.each(function() {
                ...
                function mask() {
                    var value = $input.val();
                    if (value) $input.val(maskValue(value));
                }
                function changeSign() {
                ...
                function keydownEvent(e) {
                    ...
                    if (key === 8 || key === 46 || key === 63272) {
                        ...
                        if ((key === Katrid.UI.Keyboard.keyCode.DELETE || key === Katrid.UI.Keyboard.keyCode.BACKSPACE) && !$input.val()) return false;
    ...
    this.Katrid = {
        ...
        i18n: {
            ...
            npgettext(ctx, singular, plural, count) {
                let value = Katrid.i18n.ngettext(ctx + "" + singular, ctx + "" + plural, count);
                if (value.indexOf("") !== -1) {
...
(function() {
    class Application {
        ...
        static get context() {
            if (Katrid.Actions.actionManager.mainAction) return Katrid.Actions.actionManager.mainAction.getContext();
        }
    }
...
(function() {
    ...
    class Service {
        ...
        post(name, data, params) {
            ...
            data = {
                ...
                method: name,
                params: data,
                id: Math.floor(Math.random() * 1e3 * 1e3 * 1e3)
            };
            if (Katrid.Settings.servicesProtocol === "io") {
    ...
    class Model extends Service {
        ...
        getViewInfo(data) {
            return this.post("get_view_info", {
                kwargs: data
            }).then(this.constructor._prepareFields);
        }
        async loadViews(data) {
            return this.post("load_views", {
                kwargs: data
            }).then(this.constructor._prepareFields);
        }
        getFieldsInfo(data) {
            return this.post("get_fields_info", {
                kwargs: data
            }).then(this.constructor._prepareFields);
        }
        getFieldChoices(field, term) {
            return this.post("get_field_choices", {
                args: [ field, term ]
            });
        }
        doViewAction(data) {
            return this.post("do_view_action", {
                kwargs: data
            });
        }
        write(data, params) {
            return new Promise((resolve, reject) => {
                this.post("write", {
                    ...
                    if (res.status === 500 && res.responseText) alert(res.responseText); else Katrid.Dialogs.Alerts.error(Katrid.i18n.gettext("Error saving record changes"));
                    reject(res);
                });
            });
        }
        groupBy(grouping) {
            return this.post("group_by", {
                kwargs: grouping
            });
        }
        autoReport() {
            return this.post("auto_report", {
                kwargs: {}
            });
        }
        rpc(meth, args, kwargs) {
            return this.post(meth, {
                args: args,
                kwargs: kwargs
            });
        }
    ...
    class Attachments {
        ...
        static upload(file, scope = null) {
            ...
            return $.ajax({
                ...
                if (res) for (let obj of res) scope.attachments.push(obj);
                scope.$apply();
            });
        }
    }
...
(() => {
    class Templates {
        ...
        static prepare(name, templ) {
            ...
            if (templ.tagName === "SCRIPT") return templ.innerHTML;
            return templ;
        }
        static compileTemplate(base, templ) {
            let el = $(base);
            templ = $(templ.innerHTML);
            for (let child of Array.from(templ)) if (child.tagName === "JQUERY") {
                child = $(child);
                let sel = child.attr("selector");
                let op = child.attr("operation");
                if (sel) sel = $(el).find(sel); else sel = el;
                sel[op](child[0].innerHTML);
            }
            return el[0].innerHTML;
        }
        static loadTemplates(templateCache) {
            $.get("/web/client/templates/").done(res => {
                let readTemplates = el => {
                    if (el.tagName === "TEMPLATES") Array.from(el.childNodes).map(readTemplates); else if (el.tagName === "SCRIPT") {
...
(function() {
    ...
    class Action {
        ...
        getContext() {
            let ctx;
            if (_.isString(this.info.context)) ctx = JSON.parse(this.info.context);
            if (!ctx) ctx = {};
            return ctx;
        }
        doAction(act) {
        ...
        openObject(service, id, evt) {
            if (this._unregisterHook) this._unregisterHook();
            evt.preventDefault();
            evt.stopPropagation();
            if (evt.ctrlKey) {
        ...
        backTo(index, viewType) {
            ...
            if (this._currentPath !== this._unregisterHook && Katrid.Actions.actionManager.actions.length > 1) this._unregisterHook();
            let action = Katrid.Actions.actionManager.actions[index];
            if (index === 0 && viewType === 0) return action.restore(action.searchViewType || action.viewModes[0]); else if (index === 0 && viewType === "form") return action.restore("form");
    ...
    class WindowAction extends Action {
        ...
        deleteSelection() {
            let sel = this.selection;
            if (sel.length === 1 && confirm(Katrid.i18n.gettext("Confirm delete record?")) || sel.length > 1 && confirm(Katrid.i18n.gettext("Confirm delete records?"))) {
        ...
        _setViewType(viewType) {
            ...
            if (viewType === 0) for (let v of this.viewModes) if (v !== "form") {
                viewType = v;
                break;
            }
            let data;
            ...
            if (this.viewModes.length) {
                ...
                for (let [k, v] of Object.entries(this.views)) {
                    ...
                    if (viewCls) {
                        ...
                        let s = view.render();
                        if (!_.isString(s)) s = s[0].outerHTML;
        ...
        set viewType(value) {
            if (value === this._viewType) return;
            if (!this._viewType) this.searchViewType = this.viewModes[0];
        ...
        set view(value) {
            this._view = value;
            if (this.scope) this.scope.view = value;
        }
        get view() {
            return this._view;
        }
        get template() {
            if (!this._template) this.apply();
        ...
        setSearchParams(params) {
            let p = {};
            if (this.info.domain) p = $.parseJSON(this.info.domain);
        ...
        _doViewAction(scope, viewAction, target, confirmation, prompt) {
            ...
            if (!confirmation || confirmation && confirm(confirmation)) {
                return this.model.doViewAction({
        ...
        async formButtonClick(id, meth, self) {
            const res = await this.scope.model.post(meth, {
                kwargs: {
                    id: id
                }
            });
        ...
        doBindingAction(evt) {
            this.selection;
            Katrid.Services.Actions.load($(evt.currentTarget).data("id")).then(action => {
                if (action.action_type === "ir.action.report") ReportAction.dispatchBindingAction(this, action);
            });
        }
        ...
        autoReport() {
            return this.model.autoReport().then(function(res) {
        ...
        get selection() {
            if (this._selection) return Array.from(this._selection).map(el => $(el).data("id"));
        }
    ...
    class ReportAction extends Action {
        ...
        static async dispatchBindingAction(parent, action) {
            ...
            let res = await svc.post("export_report", {
                args: [ action.id ],
                kwargs: {
                    format: format,
                    params: params
            ...
            if (res.open) return window.open(res.open);
        }
        constructor(info, scope, location) {
            super(info, scope, location);
            this.userReport = {};
        ...
        async routeUpdate(search) {
            ...
            if (this.userReport.id) {
                const svc = new Katrid.Services.Model("ir.action.report");
                let res = await svc.post("load_user_report", {
                    kwargs: {
    ...
    class ClientAction extends Action {
        ...
        execute() {
            ...
            if (tag.prototype instanceof Katrid.UI.Views.ClientView) this.tag = new tag(this); else if (_.isString(tag)) this[tag].apply(this);
        }
...
(function() {
    ...
    class DataSource {
        ...
        set loadingAction(v) {
            if (v) this.requestInterval = 0; else this.requestInterval = DEFAULT_REQUEST_INTERVAL;
        ...
        saveAndClose() {
            ...
            if (r && $.isFunction(r.promise)) {
                return r.then(res => {
                    if (res.ok && res.result) this.scope.result = res.result;
        ...
        findById(id) {
            for (let rec of this.scope.records) if (rec.id === id) return rec;
            return null;
        }
        hasKey(id) {
        ...
        refresh(data) {
            ...
            r.then(() => {
                for (let child in this.children) if (child.invalidate) {
        ...
        _validateForm(elForm, form, errorMsgs) {
            ...
            for (let errorType in form.$error) for (let child of Array.from(form.$error[errorType])) {
                if (child.$name.startsWith("grid-row-form")) elfield = this._validateForm(elForm.find("#" + child.$name), child, errorMsgs); else {
        ...
        search(params, page, fields, timeout) {
            ...
            page = page || 1;
            this.pageIndex = page;
            let {domain: domain} = this.scope.action.info;
            if (domain) {
            ...
            return new Promise((resolve, reject) => {
                let req = () => {
                    this.model.search(params).catch(res => {
                        return reject(res);
                    }).then(res => {
                        if (this.pageIndex > 1) {
                        ...
                        this.scope.$apply(() => {
                            if (res.count != null) this.recordCount = res.count;
                            let data = res.data;
                            if (this.readonly) this.scope.records = data; else this.scope.records = data.map(obj => Katrid.Data.createRecord(obj, this));
                            if (this.pageIndex === 1) {
                ...
                if ((this.requestInterval > 0 || timeout) && timeout !== false) this.pendingRequest = setTimeout(req, this.requestInterval); else req();
            });
        }
        groupBy(group) {
            if (!group) {
            ...
            this.groups = [ group ];
            return this.model.groupBy(group.context).then(res => {
                this.scope.records = [];
                    ...
                    if (this.autoLoadGrouping) {
                        (grouping => {
                            this.model.search({
                                ...
                                if (res.ok) this.scope.$apply(() => {
                                    grouping.records = res.result.data;
                                });
                            });
                        })(grouping);
                    }
        ...
        moveBy(index) {
            ...
            if (newIndex > -1 && newIndex < this.scope.records.length) this.recordIndex = newIndex;
        }
        _clearTimeout() {
            this.loading = false;
            this.loadingRecord = false;
        ...
        getNestedData() {
            let ret = {};
            for (let child of this.children) if (child.$modifiedRecords.length) {
                let res = [];
                let deleted = [];
                for (let rec of child.$modifiedRecords) {
                    if (rec.$deleted) {
                        deleted.push(rec);
                        if (rec.id !== null && rec.id !== undefined) res.push({
                            id: rec.id,
                            action: "DESTROY"
                        });
                    }
                ...
                for (let rec of child.$modifiedRecords) {
                    ...
                    if (rec.$modifiedData && !rec.$deleted && rec.$modified && deleted.indexOf(rec) === -1) {
                        ...
                        if (rec.id === null || rec.id === undefined) res.push({
                            action: "CREATE",
                            values: data
                        }); else if (rec.id !== null && rec.id !== undefined) res.push({
                            action: "UPDATE",
                            values: data
                        });
                    }
                }
                if (Object.keys(res).length > 0) ret[child.fieldName] = res;
            }
            return ret;
        }
        save(autoRefresh = true) {
            console.log("SUBMIT", this.record.$record.toObject());
            for (let child of this.children) if (child.changing) child.scope.save();
            const el = this.scope.formElement;
            if (this.validate()) {
                const data = this.record.$record.toObject();
                this.scope.form.data = data;
                let beforeSubmit = el.attr("before-submit");
                if (beforeSubmit) beforeSubmit = this.scope.$eval(beforeSubmit);
                if (data) {
                    this.uploading++;
                    return this.model.write([ data ]).then(res => {
                        this.scope.action.location.search("id", res[0]);
                        this.scope.form.$setPristine();
                        this.scope.form.$setUntouched();
                        this._pendingChanges = false;
                        this.state = DataSourceState.browsing;
                        if (autoRefresh) return this.refresh(res);
                    }).catch(error => {
                        let s = `<span>${Katrid.i18n.gettext("The following fields are invalid:")}<hr></span>`;
                        if (error.message) s = error.message; else if (error.messages) {
                            let elfield;
                            for (let fld of Object.keys(error.messages)) {
                                ...
                                if (fld.indexOf(".") > -1) {
                                    fld = fld.split(".");
                                    let subField = fld[1];
                                    for (let child of this.children) if (child.scope.fieldName === fld[0]) {
                                        field = child.scope.view.fields[subField];
                                    }
                                } else field = this.scope.view.fields[fld];
                            ...
                            if (elfield) elfield.focus();
                        }
                        return Katrid.Dialogs.Alerts.error(s);
                    }).finally(() => this.scope.$apply(() => this.uploading--));
                } else Katrid.Dialogs.Alerts.warn(Katrid.i18n.gettext("No pending changes"));
            }
        }
        _getNested(recs) {
            let res = [];
            if (recs.$deleted && recs.$deleted.recs.length) for (let rec of recs.$deleted.recs) res.push({
                id: rec.id,
                action: "DESTROY"
            });
            let vals;
            if (recs.recs.length) for (let rec of recs.recs) if (rec) {
                vals = {};
                if (rec.$created) vals = {
                    action: "CREATE",
        ...
        _getModified(data) {
            let res = {};
            if (data) for (let [k, v] of Object.entries(data)) if (v instanceof Katrid.Data.SubRecords) {
        ...
        getModifiedData(form, element, record) {
            let data = {};
            if (record.$modified) jQuery.extend(data, this._getModified(record.$modifiedData));
            if (this.record.id) data["id"] = record.id;
            return data;
        }
        get(id, timeout, apply = true, index = false) {
            ...
            return new Promise((resolve, reject) => {
                const _get = () => {
                    return this.model.getById(id).catch(res => {
                        return reject(res);
                    }).then(res => {
                        if (this._canceled || !res) return;
                        if (this.state === DataSourceState.loading) this.state = DataSourceState.browsing; else if (this.state === DataSourceState.inserting) return;
                        this.record = res.data[0];
                        if (apply) this.scope.$apply();
                        if (index !== false) this.scope.records[index] = this.record;
                ...
                if (!timeout && !this.requestInterval) return _get(); else this.pendingRequest = setTimeout(_get, timeout || this.requestInterval);
            });
        }
        insert() {
            this._clearTimeout();
            for (let child of this.children) child._clearTimeout();
            let rec = {};
            rec.$created = true;
            this.record = rec;
            return this.model.getDefaults().then(res => {
                this.scope.$apply(() => {
                    for (let child of this.children) child.scope.records = [];
                    this.state = DataSourceState.inserting;
                    this.scope.record.display_name = Katrid.i18n.gettext("(New)");
                    if (res.result) this.setValues(res.result);
                });
            });
        }
        _new() {
        ...
        setValues(values) {
            Object.entries(values).forEach(([k, v]) => {
                ...
                if (fld) fld.fromJSON(v, this); else this.scope.record[k] = v;
            });
            for (let child of this.children) child.scope.$apply();
            this.scope.$apply();
        }
        edit() {
        ...
        set state(state) {
            ...
            if (this.changing) setTimeout(() => {
                if (this.action.$element) for (let el of Array.from(this.action.$element.find("input[type!=hidden].form-field:visible"))) {
                    el = $(el);
                    if (!el.attr("readonly")) {
                        $(el).focus();
                        return;
        ...
        childByName(fieldName) {
            for (let child of this.children) {
                if (child.fieldName === fieldName) return child;
            }
        }
        get state() {
            return this._state;
        ...
        set recordId(value) {
            ...
            for (let child of this.children) child.scope.masterChanged(value);
        }
        get recordId() {
        ...
        set record(rec) {
            ...
            this._pendingChanges = false;
            if (this.scope.form) this.scope.form.$setPristine();
        }
        next() {
        ...
        set recordIndex(index) {
            ...
            if (!this.masterSource) return this.action.location.search("id", this.scope.records[index].id);
        ...
        expandGroup(index, row) {
            ...
            return this.model.search(params).then(res => {
        ...
        _applyResponse(res) {
            if (res.value) this.setValues(res.value);
            this.scope.$apply();
        }
        dispatchEvent(name, ...args) {
            this.model.rpc(name, ...args).then(res => this._applyResponse(res));
        }
        get model() {
...
(function() {
    class Record {
        ...
        $delete() {
            ...
            if (this.pk) this.setModified(); else if (this.parent.children.indexOf(this) > -1) this.parent.children.splice(this.parent.children.indexOf(this), 1);
        }
        _prepareRecord(rec) {
            let res = {};
            Object.entries(rec).map(obj => {
                if (!obj[0].startsWith("$")) res[obj[0]] = obj[1];
            });
            return res;
        }
        setModified(field) {
            if (!this.modified && this.state !== RecordState.destroyed) {
                if (this.pk) this.state = RecordState.modified; else this.state = RecordState.created;
            }
            if (field) this.dataSource.$setDirty(field);
        ...
        compare(oldValue, newValue) {
            if (_.isArray(oldValue) && _.isArray(newValue)) return oldValue.join(",") !== newValue.join(",");
        ...
        toObject() {
            ...
            if (this.pk) data.id = this.pk;
            for (let child of this.children) {
                if (!(child.scope.fieldName in data)) data[child.scope.fieldName] = [];
                if (child.state === RecordState.created) data[child.scope.fieldName].push({
                    action: "CREATE",
                    values: child.toObject()
                }); else if (child.state === RecordState.modified) data[child.scope.fieldName].push({
                    action: "UPDATE",
                    values: child.toObject()
                }); else if (child.state === RecordState.destroyed) data[child.scope.fieldName].push({
                    action: "DESTROY",
                    id: child.pk
                });
            }
    ...
    class SubRecords {
        ...
        append(rec) {
            if (this.recs.indexOf(rec) === -1) this.recs.push(rec);
        }
    }
...
(function() {
    class Field {
        ...
        createWidget(widget, scope, attrs, element) {
            if (!widget) {
                if (this.name === "status") widget = "StatusField"; else if (this.hasChoices) widget = "SelectionField";
            }
    ...
    class NumericField extends Field {
        toJSON(val) {
            if (val && _.isString(val)) return parseFloat(val);
            return val;
        }
    }
    class IntegerField extends Field {
        toJSON(val) {
            if (val && _.isString(val)) return parseInt(val);
            return val;
        }
        get paramTemplate() {
    ...
    class ManyToManyField extends ForeignKey {
        toJSON(val) {
            if (_.isArray(val)) return val.map(obj => _.isArray(obj) ? obj[0] : obj); else if (_.isString(val)) val = val.split(",");
            return val;
        }
    }
    Katrid.Data.Fields = {
...
(function() {
    ...
    ngApp.run([ "$route", "$rootScope", "$location", "$templateCache", ($route, $rootScope, $location, $templateCache) => {
    ...
    ngApp.config(function($stateProvider) {
        $stateProvider.state("menuEntry", {
            ...
            resolve: {
                action: [ "$stateParams", "$state", "$location", async ($stateParams, $state, $location) => {
            ...
            reloadOnSearch: false,
            resolve: {
                action: [ "$stateParams", "$state", "$location", async function($stateParams, $state, $location) {
                    let info = await new Katrid.Services.Model($stateParams.service).rpc("get_formview_action", [ $stateParams.id ]);
                    let model = info.model;
                    if (model instanceof Array) model = model[1];
    ...
    ngApp.controller("LoginController", function($scope, $location) {
        $scope.login = ((username, password) => {
            $scope.loading = true;
            Katrid.Services.Auth.login(username, password).then(res => {
                if (res.success) {
                    console.log(res.redirect);
                    $scope.messages = [ {
                        message: _.gettext("Loading..."),
                        type: "success"
                    } ];
                    if ($location.$$url) window.location.href = "/web/#" + $location.$$url; else if (res.redirect) window.location.href = res.redirect;
                } else {
                    $scope.loading = false;
                    $scope.messages = [ {
                        message: res.message,
    ...
    ngApp.controller("ActionController", function($scope, $compile, $state, $location, hotkeys, $element, action) {
        ...
        if (action instanceof Katrid.Actions.WindowAction) action.viewType = $location.$$search.view_type || action.viewModes[0];
        $scope.action = action;
        ...
        action.routeUpdate($location.$$search).then(() => {
    ...
    ngApp.controller("ActionController1", function($scope, $compile, $location, $route, action, reset, hotkeys) {
        ...
        let initAction = action => {
            let act, location;
            if ($scope.isDialog) location = new DialogLocation(); else location = $location;
...
(function() {
        ...
        return {
            ...
            link(scope, element, attrs, ctrl) {
                ...
                if (!element.parent("list").length) {
                    ...
                    for (let [k, v] of Object.entries(attrs)) if (k.startsWith("field")) {
                        fieldAttrs[k] = v;
    ...
    class Total {
        ...
        template(el, attrs) {
            if (attrs.type[0] === "'") return `<span>${attrs.type.substring(1, attrs.type.length - 1)}</span>`; else return `<span ng-bind="total$${attrs.field}|number:2"></span>`;
        }
        link(scope, element, attrs, controller) {
            if (attrs.type[0] !== "'") scope.$watch(`records`, newValue => {
                let total = 0;
    ...
    uiKatrid.directive("ngEsc", () => (scope, element, attrs) => element.bind("keydown keypress", event => {
        if (event.which === 27) {
            scope.$apply(() => scope.$eval(attrs.ngEsc, {
                $event: event
            }));
            event.preventDefault();
        }
    ...
    uiKatrid.directive("ajaxChoices", $location => ({
        ...
        link(scope, element, attrs, controller) {
            ...
            const cfg = {
                ajax: {
                    ...
                    results(res, page) {
                        ...
                        return {
                            results: Array.from(data).map(item => ({
                                id: item[0],
                                text: item[1]
                            })),
                            more: more
            ...
            controller.$render = (() => {
                if (controller.$viewValue) return element.select2("val", controller.$viewValue);
            });
        }
    }));
    ...
    class Decimal {
        ...
        link(scope, element, attrs, controller) {
            let precision = 2;
            if (attrs.decimalPlaces) precision = parseInt(attrs.decimalPlaces);
            ...
            controller.$parsers.push(value => {
                if (_.isString(value) && value) {
                    if (precision) value = element.maskMoney("unmasked")[0]; else {
                        value = value.replace(new RegExp(`\\${thousands}`, "g"), "");
                        value = parseInt(value);
                    }
                } else if (value) return value; else value = null;
                return value;
            });
        }
    }
    ...
    Katrid.uiKatrid.directive("foreignkey", ($compile, $controller) => ({
        ...
        link(scope, el, attrs, controller) {
            ...
            if (attrs.domain != null) domain = attrs.domain; else if (field.domain) domain = field.domain;
            if (_.isString(domain)) domain = $.parseJSON(domain);
            el.addClass("form-field");
            if (attrs.serviceName) serviceName = attrs; else serviceName = scope.action.model.name;
            ...
            let config = {
                ...
                query(query) {
                    ...
                    const f = () => {
                        let svc;
                        if (scope.model) svc = scope.model.getFieldChoices(field.name, query.term); else svc = new Katrid.Services.Model(field.model).searchName(data);
                        svc.then(res => {
                            let data = res.items;
            ...
            controller.$parsers.push(value => {
                if (value) {
                    if (_.isArray(value)) return value; else if (_.isObject(value)) return [ value.id, value.text ]; else return value;
                }
                return null;
            });
    ...
    uiKatrid.filter("m2m", () => (function(input) {
        if (_.isArray(input)) return input.map(obj => obj ? obj[1] : null).join(", ");
    }));
    ...
    uiKatrid.directive("dateInput", [ "$filter", $filter => ({
        ...
        link(scope, element, attrs, controller) {
            let setNow = () => {
                let value;
                if (attrs["type"] === "date") value = new Date().toISOString().split("T")[0]; else value = moment(new Date()).format("YYYY-MM-DD HH:mm").replace(" ", "T");
                $(element).val(value);
            ...
            element.focus(function() {
                ...
                if (/\d/.test(evt.key)) {
                    if ($(this).val() === "" && _focus) setNow();
                }
            });
            controller.$formatters.push(function(value) {
                if (value) {
                    if (attrs["type"] === "date") return new Date(moment.utc(value).format("YYYY-MM-DD") + "T00:00"); else return new Date(value);
                }
            });
            controller.$parsers.push(function(value) {
                if (_.isDate(value)) {
                    if (attrs["type"] === "date") return moment.utc(value).format("YYYY-MM-DD"); else return moment.utc(value).format("YYYY-MM-DDTHH:mm:ss");
                }
            });
        }
    }) ]);
    uiKatrid.directive("statusField", [ "$compile", "$timeout", ($compile, $timeout) => ({
        ...
        template(element, attrs) {
            return sprintf(Katrid.$templateCache.get("view.field.StatusField"), {
    ...
    uiKatrid.directive("cardDraggable", () => {
        return {
            ...
            link(scope, element, attrs, controller) {
                ...
                if (!_.isUndefined(attrs.cardItem)) cfg["receive"] = ((event, ui) => {
                    ...
                    parent.model.write([ data ]).then(res => {
                        console.log("write ok", res);
                    });
                });
                if (!_.isUndefined(attrs.cardGroup)) cfg["update"] = ((event, ui) => {
                    let ids = [];
                    ...
                    Katrid.Services.data.reorder(modelName, ids).done(res => {
                        console.log(res);
                    });
                });
    ...
    uiKatrid.setFocus = (el => {
        let e = $(el);
        if (e.data("select2")) e.select2("focus"); else el.focus();
    });
        ...
        link: (scope, el) => {
            ...
            scope.$parent.$watch("recordId", key => {
                ...
                _pendingOperation = setTimeout(() => {
                    attachment.search({
                        params: {
                            model: scope.action.model.name,
                            object_id: key
                        },
                        count: false
                    }).then(res => {
                        let r = null;
                        if (res && res.data) r = res.data;
        ...
        controller($scope, element, attrs) {
            ...
            $scope.cardAddItem = ((event, name) => {
                if (name) {
                    ...
                    scope.model.createName(name, ctx).done(res => {
                        if (res.ok) {
                            let id = res.result[0];
                            scope.model.getById(id).done(res => {
                                if (res.ok) {
...
(function() {
    let uiKatrid = Katrid.uiKatrid;
    uiKatrid.controller("TabsetController", [ "$scope", function($scope) {
        const ctrl = this;
    ...
    uiKatrid.directive("tab", [ "$parse", $parse => ({
        require: "^tabset",
        restrict: "EA",
        replace: true,
...
(function() {
    ...
    class Grid {
        ...
        link(scope, element, attrs) {
            ...
            scope.isList = true;
            if (attrs.inlineEditor === "tabular") scope.inline = "tabular"; else if (attrs.hasOwnProperty("inlineEditor")) scope.inline = "inline";
            ...
            if (lst.length) scope.model.getFieldsInfo({
                view_type: "list"
            }).then(res => {
                loadViews({
                    list: {
                ...
                scope.model.loadViews().then(res => {
            ...
            let renderDialog = function() {
                ...
                if (attrs.inline) {
                    ...
                    html = $(Katrid.$templateCache.get("view.field.OneToManyField.Dialog").replace("\x3c!-- view content --\x3e", html));
                    el = me.$compile(html)(scope);
            ...
            let loadViews = obj => {
                ...
                if (scope.inline === "tabular") onclick = ""; else if (scope.inline === "inline") onclick = "editItem($event, $index)";
            ...
            scope.addItem = function() {
                ...
                if (attrs.$attr.inlineEditor) scope.records.push(scope.record); else return scope.showDialog();
            };
            scope.addRecord = function(rec) {
                let record = Katrid.Data.createRecord({}, scope.dataSource);
                for (let [k, v] of Object.entries(rec)) record[k] = v;
            ...
            scope.showDialog = function(index) {
                ...
                if (index != null) {
                    ...
                    if (scope.records[index] && !scope.records[index].$loaded) {
                        scope.dataSource.get(scope.records[index].id, 0, false, index).then(res => {
                            res.$loaded = true;
                            scope.records[index] = res;
                            scope.dataSource.edit();
                            let currentRecord = scope.record;
                            if (res.id) for (let child of dataSource.children) {
                                child.scope.masterChanged(res.id).then(res => {
                ...
                } else scope.recordIndex = -1;
                let done = () => {
                    if (needToLoad) {
                        scope.record = scope.records[index];
                        for (let child of dataSource.children) _loadChildFromCache(child);
                        scope.$apply();
                    }
                };
            ...
            const masterChanged = scope.masterChanged = (key => {
                ...
                if (key != null) {
                    ...
                    if (key) return scope.dataSource.search(data).finally(() => scope.dataSource.state = Katrid.Data.DataSourceState.browsing);
                }
            });
    ...
    class BaseTemplate {
        getBreadcrumb(scope, viewType) {
            ...
            for (let h of Katrid.Actions.Action.history) {
                ...
                i++;
                if (Katrid.Actions.Action.history.length > i && h.viewType === "form") html += `<li><a href="javascript:void(0)" ng-click="action.backTo(${i - 1})">${h.scope.record.display_name}</a></li>`;
            }
            if (scope.action.viewType === "form") html += "<li>{{ record.display_name }}</li>";
            html += "</ol>";
            return html;
        }
        ...
        renderList(scope, element, attrs, rowClick, parentDataSource, showSelector = true) {
            ...
            if (tfoot) tfoot = `<tfoot><tr>${totals.map(t => t ? `<td class="text-right"><strong><ng-total field="${t[0]}" type="${t[1]}"></ng-total></ strong></td>` : '<td class="borderless"></td>').join("")}</tr></tfoot>`; else tfoot = "";
            let gridClass = " grid";
            if (scope.inline) gridClass += " inline-editor";
...
(function() {
    ...
    class Report {
        ...
        load(fields, params) {
            ...
            this.fields = fields;
            for (let p of fields) {
                if (p.groupable) this.groupables.push(p);
                if (p.sortable) this.sortables.push(p);
                if (p.total) this.totals.push(p);
                if (!p.autoCreate) p.autoCreate = params.includes(p.name);
            }
        }
        loadParams() {
            for (let p of Array.from(this.fields)) if (p.autoCreate) this.addParam(p.name);
        }
        addParam(paramName) {
            for (let p of Array.from(this.fields)) if (p.name === paramName) {
                p = new Param(p, this);
                this.params.push(p);
                break;
            }
        ...
        renderFields() {
            ...
            sel.append($(flds)).select2({
                tags: (() => {
                    const result2 = [];
                    for (p of Array.from(this.fields)) result2.push({
                        id: p.name,
                        text: p.label
                    });
                    return result2;
    ...
    class Param {
        ...
        getOperations() {
            return Array.from(Params.TypeOperations[this.type]).map(op => ({
                id: op,
    ...
    class Telegram {
        static export(report, format) {
            ...
            let contacts = new Katrid.Services.Model("res.partner").post("get_telegram_contacts").done(res => {
                if (res) res.map(c => sel.append(`<option value="${c[0]}">${c[1]}</option>`));
                sel.select2();
            });
...
(() => {
    let compileButtons = container => {
        return container.find("button").each((idx, btn) => {
            btn = $(btn);
            if (!btn.attr("type") || btn.attr("type") === "object") btn.attr("type", "button");
            ...
            if (!btn.attr("class")) btn.addClass("btn btn-outline-secondary");
        });
    };
    ...
    class View extends ActionView {
        getBreadcrumb() {
            ...
            for (let h of Katrid.Actions.actionManager.actions) {
                if (i === 0 && h.viewModes.length > 1) html += `<li class="breadcrumb-item"><a href="javascript:void(0)" ng-click="action.backTo(0, 0)">${h.info.display_name}</a></li>`;
                i++;
                if (Katrid.Actions.actionManager.actions.length > i && h.viewType === "form") html += `<li class="breadcrumb-item"><a href="javascript:void(0)" ng-click="action.backTo(${i - 1}, 'form')">${h.scope.record.display_name}</a></li>`;
            }
            if (this.constructor.type === "form") html += `<li class="breadcrumb-item">{{ self.display_name }}</li>`;
            return html + "</ol>";
        }
        render() {
    ...
    class Form {
        ...
        buildHeader(form) {
            ...
            if (newHeader.length) {
                ...
                compileButtons(newHeader).each((idx, btn) => headerButtons.append(btn));
            } else newHeader = $("<header></header>");
...
(function() {
    ...
    class Dialog extends Katrid.UI.Views.BaseView {
        ...
        show() {
            ...
            this.el.modal("show").on("shown.bs.modal", () => Katrid.uiKatrid.setFocus(this.el.find(".form-field").first()));
            return this.el;
        }
    }
...
(function() {
    ...
    class Field {
        ...
        constructor(scope, attrs, field, element) {
            ...
            this.spanPrefix = "";
            if (field.depends != null && field.depends.length) scope.dataSource.addFieldWatcher(field);
            if (attrs.ngShow) this.templAttrs["ng-show"] = attrs.ngShow;
            if (attrs.ngReadonly || field.readonly) this.templAttrs["ng-readonly"] = attrs.ngReadonly || field.readonly;
            if (field.attrs) for (let k of field.attrs) {
                v = field.attrs[k];
            ...
            let cols = attrs.cols;
            if (!cols) {
                if (field.type === "CharField") if (field.max_length && field.max_length < 30) cols = 3;
                if (!cols) cols = DEFAULT_COLS[field.type] || 6;
            }
            this.col = cols;
            this.classes = [ "form-field" ];
            if (field.onchange) scope.$watch();
        }
        fieldChangeEvent() {}
        get caption() {
        ...
        renderTo(templTag, inplaceEditor = false, cls = "") {
            let templAttrs = [];
            for (let [k, v] of Object.entries(this.templAttrs)) templAttrs.push(k + "=" + '"' + v + '"');
            if (inplaceEditor) return `<${templTag} class="${cls}" ${templAttrs.join("")}>${this.template(this.scope, this.element, this.attrs, this.field)}</${templTag}>`;
            return `<${templTag} class="${this.field.type} section-field-${this.field.name} form-group" ${templAttrs.join("")}>` + this.template(this.scope, this.element, this.attrs, this.field) + `</${templTag}>`;
        }
        get ngModel() {
            return `record.${this.field.name}`;
        }
        get id() {
            if (!this._id) this._id = ++WIDGET_COUNT;
        ...
        widgetAttrs() {
            ...
            if (!_.isUndefined(this.attrs.$attr)) for (let attr of Object.keys(this.attrs.$attr)) {
                ...
                if (!attrName.startsWith("container-") && attr !== "ngShow" && attr !== "ngReadonly") {
                    ...
                    } else if (attrName === "class") this.classes.push(v);
                    r[attrName] = v;
                }
            }
            if (this.attrs.readonly != null || this.field.readonly) r["readonly"] = "";
            if (this.classes) r["class"] = this.classes.join(" ");
            return r;
        }
        ...
        labelTemplate() {
            ...
            if (this.attrs.nolabel === "placeholder") {
                ...
                return "";
            } else if (!_.isUndefined(this.attrs.nolabel)) return "";
            return `<label for="${this.id}" class="form-label">${label}</label>`;
        }
        get emptyText() {
            if (this.inplaceEditor) return "";
            return "--";
        }
        get readOnlyClass() {
            if (this.inplaceEditor || this.spanPrefix === "::") return "grid-field-readonly";
        ...
        widgetTemplate() {
            ...
            if (inner) html += inner + `</${this.constructor.tag}>`;
            return html;
        }
        template() {
            let label = "";
            ...
            if (this.inline === "inline") widget = `<div ng-if="dataSource.changing && dataSource.recordIndex === $index">${widget}</div>`;
        ...
        _td(cls) {
            let content;
            if (this.inplaceEditor) content = this._gridEditor(cls); else {
                this.spanPrefix = "::";
    ...
    class InputWidget extends Field {
        ...
        widgetTemplate() {
            ...
            if (prependIcon) return `<label class="prepend-icon"><i class="icon ${prependIcon}"></i>${html}</label>`;
            const inner = this.innerHtml();
            if (inner) html += inner + `</${this.constructor.tag}>`;
            return html;
        }
    ...
    class StringField extends InputWidget {
        widgetAttrs() {
            ...
            if (this.field.maxLength) attributes["maxlength"] = this.field.maxLength.toString();
            return attributes;
        }
    }
    class NumericField extends InputWidget {
        ...
        get type() {
            if (Katrid.Settings.UI.isMobile) return "number";
            return "text";
        }
        spanTemplate() {
    ...
    class ForeignKey extends Field {
        ...
        spanTemplate() {
            let allowOpen = true;
            if (this.attrs.allowOpen != null && this.attrs.allowOpen === "false" || this.attrs.allowOpen == null && this.field.attrs && this.field.attrs["allow-open"] === false) allowOpen = false;
            if (!allowOpen || this.inList) return `<span class="${this.readOnlyClass}"><a href="javascript:void(0)">{{ ${this.spanPrefix}record.${this.field.name}[1] || '${this.emptyText}' }}</a></span>`;
    ...
    class FloatField extends NumericField {
        static get tag() {
            if (Katrid.Settings.UI.isMobile) return "input";
            return "input decimal";
        }
        get type() {
            if (Katrid.Settings.UI.isMobile) return "number";
        ...
        _tdContent() {
            ...
            if (decimalPlaces) filter`number:${decimalPlaces}`; else filter = `numberFormat:${this.element.attr("max-digits") || 6}`;
    ...
    class DecimalField extends FloatField {
        spanTemplate() {
            ...
            let fmt = "number";
            if (maxDigits) fmt = "numberFormat"; else maxDigits = this.attrs.decimalPlaces || 2;
        ...
        _tdContent(cls) {
            ...
            if (maxDigits) return `<td class="${cls}">{{::record.${this.field.name}|numberFormat:${maxDigits} }}${this._gridEditor()}</td>`; else {
                maxDigits = 2;
    ...
    class BooleanField extends InputWidget {
        ...
        labelTemplate() {
            if (this.field.help_text) return super.labelTemplate();
...
(function() {
    class SearchMenu {
        ...
        reset() {
            for (let i of this.searchView.items) if (i.children) {
                this.collapse(i);
                i.reset();
            }
        }
    ...
    class SearchItem {
        ...
        template() {
            let s = "";
            if (this.expandable) s = `<a class="expandable" href="#"></a>`;
            if (this.value) s = `<a class="search-menu-item indent" href="#">${this.value[1]}</a>`; else s += `<a href="#" class="search-menu-item">${this.templateLabel()}</a>`;
            return `<li>${s}</li>`;
        }
        link(action, $compile, parent) {
            ...
            } else html.appendTo(this.parent);
            this.element = html;
    ...
    class SearchFilter extends SearchItem {
        ...
        link(scope, $compile, parent) {
            ...
            el.click(function(evt) {
                evt.preventDefault();
                let e = $(this);
                if (me.facet) me.remove(); else me.select();
            });
            return ul.append(el);
        }
        select(el) {
    ...
    class SearchView extends Katrid.UI.Widgets.Widget {
        ...
        createMenu(scope, el, parent) {
            const menu = new SearchMenu(el, parent, {
        ...
        link(scope, el, attrs, controller, $compile) {
            ...
            for (let item of Array.from(this.viewContent.children())) this.loadItem($(item));
        }
        ...
        load(items) {
            for (let i of items) new FacetView(this.items[i.id], i.facet.values).load(this);
        }
        renderFacets() {
            for (let item of this.query.items) if (!item.facet) new FacetView(item).render(this);
        }
        first() {
    ...
    class Comments {
        ...
        async masterChanged(key) {
            if (key) {
                ...
                if (this.scope.$parent.record) return svc.post("get_messages", {
        ...
        async _sendMesage(msg, attachments) {
            ...
            let msgs = await this.model.post("post_message", {
        ...
        postMessage(msg) {
            if (this.scope.files.length) {
                ...
                var me = this;
                Katrid.Services.Attachments.upload({
                    files: files
                }, this.scope).done(res => {
                    me._sendMesage(msg, res);
                });
            } else this._sendMesage(msg);
        }
    }
    ...
    Katrid.uiKatrid.directive("mailComments", () => ({
        ...
        controller: $scope => {
            ...
            $scope.attachFile = (file => {
                for (let f of file.files) $scope.files.push({
                    name: f.name,
                    type: f.type,
                    file: f
                });
...
(function() {
    ...
    class Chart extends Katrid.UI.Widgets.Component {
        ...
        async link(scope, el, attrs) {
            let res, chart;
            let observe = async () => {
                if (_.isUndefined(attrs.url)) res = await Katrid.Services.Query.read(attrs.queryId); else res = await $.ajax({
                    url: attrs.url,
                    type: "get"
                });
    ...
    class Query extends Katrid.UI.Widgets.Component {
        ...
        link(scope, el, attrs) {
            ...
            let r;
            if (_.isUndefined(attrs.url)) r = Katrid.Services.Query.read(attrs.id); else r = $.get(attrs.url);
            r.done(res => {
...
(function() {
            ...
            if (cmd) {
                ...
                for (let obj of this.commands) {
                    if (obj.name.toLocaleLowerCase() === cmd.toLocaleLowerCase()) {
                        if (obj.command) eval(obj.command); else if (obj.href) window.location.href = obj.href;
                        break;
                    }
                }
            }
    ...
    if (Katrid.Settings.Speech.enabled) {
        ...
        model.search().done(res => {
            if (res.ok) for (let cmd of res.result.data) Katrid.Speech.voiceCommand.commands.push({
                name: cmd.name,
                command: cmd.command
            });
        });
