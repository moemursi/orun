...
class GenericForeignKey(object):
    ...
    def get_forward_related_filter(self, obj):
        ...
        return {
            ...
            self.ct_field: Model.objects.get_for_model(obj).pk,
        }
    ...
    def _check_content_type_field(self):
        ...
        valid content_type field (is a ForeignKey to Model).
        """
        try:
            ...
            if not isinstance(field, models.ForeignKey):
                return [
                    checks.Error(
                        ...
                        hint=(
                            ...
                            "'models.Model' as the 'content_type' field."
                        ),
                        obj=self,
                        id='models.E003',
                    )
                ...
                return [
                    checks.Error(
                        "'%s.%s' is not a ForeignKey to 'models.Model'." % (
                            self.model._meta.object_name, self.ct_field
                        ),
                        hint=(
                            "GenericForeignKeys must use a ForeignKey to "
    ...
    def get_content_type(self, obj=None, id=None, using=None):
        if obj is not None:
            return Model.objects.db_manager(obj._state.db).get_for_model(
                obj, for_concrete_model=self.for_concrete_model)
        elif id is not None:
            return Model.objects.db_manager(using).get_for_id(id)
        else:
...
class GenericRelation(ForeignObject):
    ...
    def get_content_type(self):
        ...
        """
        return Model.objects.get_for_model(self.model,
                                                 for_concrete_model=self.for_concrete_model)

    def get_extra_restriction(self, where_class, alias, remote_alias):
        field = self.remote_field.model._meta.get_field(self.content_type_field_name)
        model_pk = self.get_content_type().pk
        cond = where_class()
        lookup = field.get_lookup('exact')(field.get_col(remote_alias), model_pk)
        cond.add(lookup, 'AND')
        return cond
    ...
    def bulk_related_objects(self, objs, using=DEFAULT_DB_ALIAS):
        ...
        return self.remote_field.model._base_manager.db_manager(using).filter(**{
            "%s__pk" % self.content_type_field_name: Model.objects.db_manager(using).get_for_model(
...
def create_generic_related_manager(superclass, rel):
    ...
    class GenericRelatedObjectManager(superclass):
        def __init__(self, instance=None):
            ...
            content_type = Model.objects.db_manager(instance._state.db).get_for_model(
