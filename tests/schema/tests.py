...
from orun.db.models.deletion import CASCADE
...
class SchemaTests(TransactionTestCase):
    ...
    def tearDown(self):
        ...
        if 'schema' in new_apps.module_models:
            for model in self.local_models:
                for many_to_many in model._meta.many_to_many:
                    through = many_to_many.remote_field.through
                    if through and through._meta.auto_created:
                        del new_apps[through._meta.name]
                del new_apps[model._meta.name]
    ...
    def test_alter_o2o_to_fk(self):
        ...
        BookWithO2O.objects.create(author=author, title="Orun 1", pub_date=datetime.datetime.now())
        with self.assertRaises(IntegrityError):
            BookWithO2O.objects.create(author=author, title="Orun 2", pub_date=datetime.datetime.now())
        ...
        Book.objects.create(author=author, title="Orun 1", pub_date=datetime.datetime.now())
        Book.objects.create(author=author, title="Orun 2", pub_date=datetime.datetime.now())
    ...
    def _test_m2m_create_through(self, M2MFieldClass):
        ...
        class LocalTagThrough(Model):
            book = ForeignKey("schema.local.book.with.m2m.through", CASCADE)
            tag = ForeignKey("schema.tag.m2m.test", CASCADE)
    ...
    def _test_m2m(self, M2MFieldClass):
        ...
        with connection.schema_editor() as editor:
            ...
            editor.create_model(TagM2MTest)
        # Create an M2M field
        new_field = M2MFieldClass(TagM2MTest, related_name="authors")
        new_field.contribute_to_class(LocalAuthorWithM2M, "tags")
        ...
        del new_apps[new_field.remote_field.through._meta.name]
        opts._expire_cache()
    ...
    def _test_m2m_through_alter(self, M2MFieldClass):
        ...
        class LocalAuthorTag(Model):
            author = ForeignKey("schema.local.author.with.m2m.through", CASCADE)
            tag = ForeignKey("schema.tag.m2m.test", CASCADE)
        ...
        class LocalAuthorWithM2MThrough(Model):
            name = CharField(max_length=255)
            tags = M2MFieldClass("schema.tag.m2m.test", related_name="authors", through=LocalAuthorTag)
    ...
    def test_create_index_together(self):
        ...
        self.assertEqual(
            ...
            any(
                c["index"]
                for c in self.get_constraints("schema_tag_indexed").values()
                if c['columns'] == ["slug", "title"]
            ),
        )
    ...
    @mock.patch('orun.db.backends.base.schema.datetime')
    @mock.patch('orun.db.backends.base.schema.timezone')
